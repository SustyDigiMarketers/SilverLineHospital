...
const LinkSection: React.FC<{ title: string, links: { name: string; href: string }[], handleLinkClick: (e: React.MouseEvent<HTMLAnchorElement>, href: string) => void }> = ({ title, links, handleLinkClick }) => {
  return (
        ...
        {links.map(link => (
          <li key={link.name}>
            <a 
              ...
              className="hover:text-[#00B5A5] transition-colors rounded-md focus:outline-none focus:ring-1 focus:ring-[#00B5A5] block py-1"
            >
              {link.name}
            </a>
          </li>
    ...
    <footer className="bg-[#0E2A47] text-gray-300">
      <div className="container mx-auto max-w-7xl px-4 sm:px-6 py-16">
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-12">
          ...
          {/* Columns for Links - Dropdowns Removed */}
          <LinkSection title="Quick Links" links={quickLinks} handleLinkClick={handleLinkClick} />
          <LinkSection title="For Patients" links={forPatientsLinks} handleLinkClick={handleLinkClick} />
