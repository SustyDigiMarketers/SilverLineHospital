  ...
  return (
    <section className="relative z-20">
      <div className="container mx-auto px-4 relative -mt-10 animate-on-scroll fade-in-up">
        <div className="max-w-5xl mx-auto bg-white/90 backdrop-blur-xl rounded-full shadow-[0_20px_50px_rgba(0,0,0,0.1)] flex flex-col md:flex-row overflow-hidden transition-all duration-500 hover:shadow-[0_25px_60px_rgba(0,0,0,0.15)] border border-white/50">
          {navItems.map((item, index) => (
            <a
              key={index}
              href={item.href}
              onClick={(e) => handleNavClick(e, item.href)}
              className={`flex-1 py-4 md:py-6 flex items-center justify-center gap-4 transition-all duration-300 group ${item.hoverClass} border-b md:border-b-0 md:border-r border-gray-100/50 last:border-0`}
            >
              <div className={`p-2.5 rounded-xl transition-all duration-500 group-hover:scale-110 group-hover:rotate-3 ${item.iconClass}`}>
              ...
              <EditableText
                ...
                className={`text-[#0E2A47] group-hover:${item.activeColor} text-lg md:text-xl font-bold tracking-tight transition-colors duration-300`}
              />
            </a>
          ))}
        </div>
